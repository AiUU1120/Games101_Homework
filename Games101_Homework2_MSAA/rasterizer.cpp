...
void rst::rasterizer::rasterize_triangle(const Triangle& t)
{
    ...
    for (int x = minX; x <= maxX; ++x)
    {
        for (int y = minY; y <= maxY; ++y)
        {
            ...
            static const float sub_offsets[4][2] = {
                {0.25f, 0.25f}, {0.75f, 0.25f},
                {0.25f, 0.75f}, {0.75f, 0.75f}
            };
            for (int s = 0; s < 4; s++)
            {
                const float sx = x + sub_offsets[s][0];
                const float sy = y + sub_offsets[s][1];
                if (insideTriangle(sx + 0.5f, sy + 0.5f, t.v))
                {
                    is_sampled[ind] = true;
                    ...
                    if (depth_buf_msaa[ind][s] > z_interpolated)
                    {
                        depth_buf_msaa[ind][s] = z_interpolated;
                        frame_buf_msaa[ind][s] = t.getColor();
                    }
                }
            }
            if (is_sampled[ind])
            {
                Eigen::Vector3f avg_color = Eigen::Vector3f::Zero();
                for (int s = 0; s < 4; s++)
                {
                    avg_color += frame_buf_msaa[ind][s];
                }
                avg_color /= 4.0f;
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    ...
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        ...
        for (auto& pixel : depth_buf_msaa)
        {
            for (auto& depth : pixel)
            {
    ...
    std::fill(is_sampled.begin(), is_sampled.end(), false);
}
